...
public class ExampleUCT extends AI
{
	...
	private static final boolean VERBOSE = false; // for convinience

	public Node root;
	public String analysisReport = null;
	//-------------------------------------------------------------------------
	...
	)
	{
		...
		analysisReport = String.format("%s v=%,d",friendlyName, root.visitCount);
	...
	public static Move finalMoveSelection(final Node rootNode)
	{
        ...
        for (int i = 0; i < numChildren; ++i) 
        {
            ...
            )
            {
				...
				if (VERBOSE) System.out.println("UCTNode v="+rootNode.visitCount);
